...
words:[‘تایرە’,‘یارئ شەبەکە’,‘تێنس’,‘زۆرانبازی’,‘مەلەکردن’,‘بەرشەلۆنە’,‘یاریگا’,‘نابژیوان’,‘رۆبێرتۆ کارلوس’,‘ریـال’,‘رۆنالدۆ’,‘مودەڕیب’,‘کارتی سووری تۆپی پێ’,‘لاپچین’,‘گۆڵ’,‘قازانج’,‘یانەی هەولێر’,‘فریز’]
},
{
...
words:[‘مۆبایل’,‘لاپتۆپ’,‘تابلێت’,‘درۆن’,‘رۆبۆت’,‘کامێرا’,‘شاشە’,‘کیبۆرد’,‘ئینتەرنێت’,‘ئەتاری پلەی دوو’,‘رسم’,‘وایەر شەحن’,‘پاتری’,‘سناپ’,‘تیک تۆک’,‘هاردسک’,‘ئیتفۆن’,‘وێبسایت’]
},
{
...
words:[‘دکتۆر’,‘مامۆستا’,‘سەرتاش’,‘ئەندازیار’,‘پەرستار’,‘پەرستار’,‘ئاکتەر’,‘خوێندکار’,‘سایەق بالیف’,‘دز’,‘شامپۆ’,‘دادگا’,‘پۆلیس’,‘سەرۆک’,‘کەناس’,‘خوێندکار’,‘وتاربێژ’,‘نووسەر’]
},
{
...
words:[‘کەرەستە’,‘چادر’,‘سەندوق’,‘قوفل’,‘کتێب’,‘خازنە’,‘سووچ’,‘دووری’,‘نووسەر’,‘سیفر’,‘خەڵک’,‘ساتک’,‘میز’,‘کورسی’,‘پەنجەرە’,‘شووشە’,‘فرێزەر’,‘بالیف’]
},
{
...
row.innerHTML = `<div class="player-num">${i+1}</div> <input type="text" placeholder="ناوی یاریزانی ${i+1}" value="${name}" oninput="G.players[${i}]=this.value" /> ${G.players.length > 2 ?`<button class="del-btn" onclick="removePlayer(${i}); sndClick();">
...
if (isSpy) {
badge.innerHTML = `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"> <circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1.5"/> <path d="M4 6c0-1.105.895-2 2-2s2 .895 2 2-.895 2-2 2-2-.895-2-2z" fill="currentColor"/> </svg> فریودەر`;
word.textContent = ‘تۆ فریودەریت!’;
hint.textContent = ‘وشەکە نازانی — بفریوێنە’;
warn.textContent = ‘زیرەکانە قسە بکە — وانەیەک بدە’;
} else {
badge.innerHTML = `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"> <path d="M2 6l3 3 5-5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/> </svg> وشەی نهێنی`;
...
row.innerHTML = `<div class="tally-name">${p}</div> <div class="tally-track"> <div class="tally-fill" style="width:0%" data-pct="${pct}"> ${counts[p] > 0 ? counts[p] : ''} </div> </div>`;
...
iconSvg.innerHTML = ` <circle cx="20" cy="16" r="6" stroke="#ff4d6d" stroke-width="2.5"/> <path d="M8 34c0-6.627 5.373-12 12-12s12 5.373 12 12" stroke="#ff4d6d" stroke-width="2.5" stroke-linecap="round"/> <path d="M28 10l4 4M32 10l-4 4" stroke="#ff4d6d" stroke-width="2" stroke-linecap="round"/>`;
}
...
row.innerHTML = `<span class="rr-name">${p}</span> <span class="rr-word">${isSpy ? '—' : G.word}</span> <span class="rr-role ${isSpy ? 'role-spy' : 'role-normal'}">${isSpy ? 'فریودەر' : 'باش'}</span>`;
...
piece.style.cssText = `left: ${Math.random()*100}%; top: -10px; background: ${colors[Math.floor(Math.random()*colors.length)]}; width: ${6 + Math.random()*6}px; height: ${6 + Math.random()*6}px; border-radius: ${Math.random() > 0.5 ? '50%' : '2px'}; animation-duration: ${1.5 + Math.random()*2}s; animation-delay: ${Math.random()*0.8}s;`;
